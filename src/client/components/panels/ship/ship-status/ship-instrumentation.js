  ...
  return (
    <div ref={scaledWrapper} style={{ position: 'fixed', pointerEvents: 'none', top: '14.25rem', bottom: '2rem', right: '1rem', left: '5rem', xoverflow: 'hidden' }}>
      ...
      >
        ...
        <table
          className='ship-panel__ship-stats visible-medium table--layout'
          style={{
            marginBottom: '4rem'
          }}
        >
...
function NavigationInstrumentation ({ ship, cmdrStatus }) {
  ...
  return (
    <div
      className={`ship-panel__navigation-instrumentation ${panelActive ? '--on-board' : ' text-muted'} ${panelActive && typeof cmdrStatus?.heading === 'number' ? '--active' : ''} text-uppercase`}
      style={{
        minHeight: '13rem',
        minWidth: '13rem'
      }}
