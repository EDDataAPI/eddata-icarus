...
function LogPageContent () {
  ...
  useEffect(() => {
    ...
    sendEvent('getLogEntries', { count: 100 }).then(newLogEntries => {
  ...
  useEffect(() => {
    return eventListener('newLogEntry', async (newLogEntry) => {
...
export default dynamic(() => Promise.resolve(LogPageContent), {
  ssr: false,
  loading: () => null
})
