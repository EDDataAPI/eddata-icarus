  ...
  load ({
    ...
    return new Promise((resolve) => {
      this.#getFiles().then(async (filesResult) => {
        ...
        const files = file ? [file] : filesResult // Only determine a minimum timestamp if number of days specified and on
          ...
          for (const file of files) {
            await retry(() => { // Auto-retry on failure (write in progress)
              ...
              for (const log of logs) {
                if (!newestTimestamp || (Date.parse(log.timestamp) > Date.parse(newestTimestamp))) { newestTimestamp = log.timestamp }

                if (!oldestTimestamp || (Date.parse(log.timestamp) < Date.parse(oldestTimestamp))) { oldestTimestamp = log.timestamp }
              }
            }, {
              retries: 10
            })
        ...
        for (const log of logs) {
          ...
          if (!this.lastActiveTimestamp) { this.lastActiveTimestamp = eventTimestamp }

          if (Date.parse(eventTimestamp) > Date.parse(this.lastActiveTimestamp)) { this.lastActiveTimestamp = eventTimestamp }
  ...
  watch (callback) {
    const watchFiles = async () => {
      ...
      for (const file of files) {
        ...
        if (!this.files[file.name].watch && file.name === activeLogFile.name) { this.files[file.name].watch = this.#watchFile(file, callback) }
      }
